...
const InvestorCompanies = () => {
  ...
  const [statusMessage, setStatusMessage] = useState(""); // ✅ for showing success message
   
  const handleSubmit = async (e) => {
    e.preventDefault();
    setStatusMessage(""); // clear message before submit
    ...
    try {
      ...
      if (res.ok) {
        setStatusMessage("✅ Your request has been submitted. Our team will call you shortly.");
        setLead({ name: "", phone: "" });
      } else {
  ...
  return (
    <section className="bg-white py-12 px-4 md:px-16">
      ...
      {statusMessage && (
        <p className="text-center text-sm text-green-700 bg-green-100 border border-green-300 rounded px-4 py-2 mb-4">
          {statusMessage}
        </p>
      )}
